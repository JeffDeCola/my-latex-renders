...
\resizebox{20cm}{!}{
    ...
    \begin{tikzpicture}[label distance=2mm]
        ...
        % INPUTS - JK FLIP-FLOP
        \node[my-branch] (J) at ($(T) + (2, 0)$)  {}; % *****POSITION*****
        \coordinate[] (CLK1) at ($(J) + (0, -2)$) {}; % *****POSITION, COORDINATE AND CHANGE NAME*****
        \coordinate[] (K)    at ($(J) + (0, -4)$) {}; % *****POSITION and COORDINATE*****
        ...
        \node[] (Q)    at ($(NAND1OUT) + (4.2, 0)$) {\normalsize $q$};
        \node[] (QBAR) at ($(NAND2OUT) + (4.2, 0)$) {\normalsize $\bar{q}$};
      ...
      % T FLIP-FLOP (CONTINUED) -----------------------------------------------------------------------
        ...
        % DRAW DOTTED LINE BOX AROUND T FLIP-FLOP
        \draw[thick, dashed, purple] ($(NAND3IN1) + (-2.5, 2.6)$) rectangle ($(NAND2OUT) + (3.2, -3.2)$);
        \node[purple] at ($(NAND1) + (2.1, 2.6)$) {\normalsize T FLIP-FLOP};
